<div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <h2 class="text-title-md2 font-bold text-black dark:text-white">Tables</h2>
    </div>
    <div class="flex flex-col gap-10">
        <div class="rounded-sm border border-stroke bg-white px-5 pb-2.5 pt-6 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
            <div class="max-w-full overflow-x-auto">
                        ...
                        @foreach ($product_cats as $product_cat )
                            <tr class="text-center"  wire:key="task-{{ $product_cat->id }}">
                                <td class="border-b border-[#eee] px-4 py-5 pl-9 dark:border-strokedark xl:pl-11">
                                    <input type="checkbox" wire:model="items" value="{{$product_cat["id"]}}" @if(isset($product_cat->id)) checked @endif>
                                </td>
                                <td class="border-b border-[#eee] px-4 py-5 pl-9 dark:border-strokedark xl:pl-11">
                ...
                {{$product_cats->links()}}
                <button class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 border border-red-700 rounded mt-5" wire:click="action_all('delete_all')">حذف کلی</button>
                <button class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 border border-blue-700 rounded mt-5" wire:click="action_all('state_all')">تغییر وضعیت</button>
            </div>
        </div>
    </div>
</div>