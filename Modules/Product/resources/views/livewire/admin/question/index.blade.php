                    ...
                    <tbody>
                        @if($questions[0])
                            @foreach ($questions as $question )
                                <tr class="text-center"  wire:key="task-{{ $question->id }}">
                                    <td class="border-b border-[#eee] px-4 py-5 pl-9 dark:border-strokedark xl:pl-11">
                                        <input type="checkbox" value="{{$question->id}}" wire:model="items" >
                                    </td>
                                    <td class="border-b border-[#eee] px-4 py-5 pl-9 dark:border-strokedark xl:pl-11">
                                    ...
                                    <td class="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                                        <p class="text-black dark:text-white">{{$question->dateConvert()}}</p>
                                    </td>
                                    <td class="border-b border-[#eee] px-4 py-5 dark:border-strokedark flex justify-center">
                                        <x-base::admin.state :state="$question->state->value" :id="$question->id" methodName="changeState({{$question->id}})"></x-base::admin.state>
                                    </td>
                                    <td class="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                                        <div class="flex items-center justify-center space-x-3.5 mx-5">
                                            <a wire:navigate href="{{route("admin.question.edit",['question'=>$question->id])}}" class="mx-3 rounded-md bg-green-500 hover:bg-green-700 border-green-700 px-2 py-2" >
                                            ...
                                            <button class="rounded-md bg-red-500 hover:bg-red-700 border-red-700 px-2 py-2" wire:click="delete({{$question->id}})"   >
