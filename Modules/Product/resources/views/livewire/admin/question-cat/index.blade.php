                    ...
                    <tbody>
                        @if($questionCats[0])
                            @foreach ($questionCats as $questionCat )
                                <tr class="text-center"  wire:key="task-{{ $questionCat->id }}">
                                    <td class="border-b border-[#eee] px-4 py-5 pl-9 dark:border-strokedark xl:pl-11">
                                        <input type="checkbox" value="{{$questionCat->id}}" wire:model="items" >
                                    </td>
                                    <td class="border-b border-[#eee] px-4 py-5 pl-9 dark:border-strokedark xl:pl-11">
                                    ...
                                    <td class="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                                        <p class="text-black dark:text-white">{{$questionCat->dateConvert()}}</p>
                                    </td>
                                    {{-- <td class="border-b border-[#eee] px-4 py-5 dark:border-strokedark flex justify-center">
                                        <x-base::admin.state :state="$questionCat->state->value" :id="$questionCat->id" methodName="changeState({{$questionCat->id}})"></x-base::admin.state>
                                    </td> --}}
                                    <td class="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                                        <div class="flex items-center justify-center space-x-3.5">
                                            <a wire:navigate href="{{route("admin.questioncat.edit",['questioncat'=>$questionCat->id])}}" class="mx-3 rounded-md bg-green-500 hover:bg-green-700 border-green-700 px-2 py-2" >
                                            ...
                                            <button class="rounded-md bg-red-500 hover:bg-red-700 border-red-700 px-2 py-2" wire:click="delete({{$questionCat->id}})"   >
