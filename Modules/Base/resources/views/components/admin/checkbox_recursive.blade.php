@props(['title'=>'','name'=>'','options'=>[],'key'=>'','val'=>'','value'=>'','placeholder'=>'انتخاب کنید','class'=>'w-50','ClickAction'=>'','select2'=>true,'id'=>'','sub_method'=>''])
    ...
    <div class="flex flex-col gap-5.5 p-3">
        @foreach($options as $cats)
            ...
            <label for="{{$cats->id}}" class="flex cursor-pointer select-none items-center text-sm font-medium">
                <div class="relative">
                    <input type="checkbox" wire:model="{{$name}}" id="{{$cats->id}}" value="{{$cats->id}}"
