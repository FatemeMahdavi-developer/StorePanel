...
<div id="FileUpload" class="relative mb-5.5 block w-full cursor-pointer appearance-none rounded border border-dashed border-primary bg-gray px-4 py-4 dark:bg-meta-4 sm:py-7.5">
    ...
    <input type="file" wire:model="{{$name}}"  class="absolute inset-0 z-50 m-0 h-full w-full cursor-pointer p-0 opacity-0 outline-none">
    ...
    <div class="flex flex-col items-center justify-center space-y-3">
        ...
        @elseif(!is_null($value) && !empty($value))
            @if(in_array(pathinfo($value,PATHINFO_EXTENSION),config('base.img_extension')))
                <span class="flex items-center justify-center rounded-full">
                    <img src="{{$value}}" class="rounded-full h-[100px]">
                    {{-- <a href="javascript:void(0)" wire:click="deleteImage"class="delete_img"> --}}
                        <img src="{{asset("admin/assets/images/icon/delete.svg")}}">
                    {{-- </a> --}}
                </span>
            ...
            @endif
        @else
            <span class="flex h-[100px] w-[100px] items-center justify-center rounded-full border border-stroke bg-white dark:border-strokedark dark:bg-boxdark">
                <svg width="30px" height="30px" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"  class="p-1">
        ...
        @if($title)
            <p class="text-sm font-medium">آپلود {{$title}}</p>
        @endif

        <p class="mt-1.5 text-sm font-medium">{{strtoupper(implode(',',config('base.img_extension')))}}</p>
        <p class="text-sm font-medium">800x350 px</p>
